...
def crear_proyecto(request):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.error(request, 'Hubo un error al crear el proyecto: ' + str(e) + '.')
...
def editar_proyecto(request):
    ...
    try:
        ...
        if p.n_expediente != None:
            p.n_expediente = p.n_expediente + '|' + request.POST['n_expediente']
        else:
            p.n_expediente = request.POST['n_expediente']
        p.save()
        messages.success(request, 'Proyecto editado con exito.')
    except Exception as e:
        messages.error(request, 'Hubo un error al editar el proyecto: ' + str(e) + '.')
