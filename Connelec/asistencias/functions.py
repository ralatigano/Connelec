...
#                 lista_entradas.append(datetime.strptime(
#                     str(registro.hora.hour)+':'+str(registro.hora.minute), '%H:%M'))  # registro.hora
#                 cont_entradas += 1
#                 tipo_reg_ant = 'Entrada'
#             else:
#                 lista_entradas[cont_entradas] = datetime.strptime(
#                     str(registro.hora.hour)+':'+str(registro.hora.minute), '%H:%M')  # registro.hora
#                 tipo_reg_ant = 'Entrada'
#         else:
...
#                 lista_salidas.append(datetime.strptime(
#                     str(registro.hora.hour)+':'+str(registro.hora.minute), '%H:%M'))  # registro.hora
#                 cont_salidas += 1
#                 tipo_reg_ant = 'Salida'
#             else:
#                 lista_salidas[cont_salidas] = datetime.strptime(
#                     str(registro.hora.hour)+':'+str(registro.hora.minute), '%H:%M')  # registro.hora
#                 tipo_reg_ant = 'Salida'
...
#                 tiempo_horas += (hora_actual_obj -
#                                  lista_entradas[i]).total_seconds() // 3600
#                 tiempo_mins += (hora_actual_obj -
#                                 lista_entradas[i]).total_seconds() % 3600 // 60
#             else:
#                 tiempo_horas += (lista_salidas[i] -
#                                  lista_entradas[i]).total_seconds() // 3600
#                 tiempo_mins += (lista_salidas[i] - lista_entradas[i]
...
#     if f1 > f2:
#         mensaje = [
#             'La fecha inicial no puede ser posterior a la fecha final', True]
#         return mensaje
...
#     mensaje = [
#         f'{nombre} trabajo {tiempo_total} horas en el periodo indicado.', False]
#     return mensaje
...
# Funciones para generar rutas de archivo
